  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        ...
        <Stat label="Thành công hôm nay" value="88.9%" note="Tỷ lệ" tone="good" />
      </div>

      <Panel
        title="Pipeline"
        actions={<label className="route-search"><Search /><input placeholder="Tìm kiếm..." /></label>}
      >
            ...
            <tbody>
              {pipelines.map((p) => (
                <tr key={p.id}>
                  ...
                  <td><Pill tone={p.status === 'running' ? 'warning' : p.status === 'blocked' ? 'bad' : 'good'}>{p.status === 'running' ? 'Đang chạy' : p.status === 'blocked' ? 'Bị chặn' : 'Bình thường'}</Pill></td>
